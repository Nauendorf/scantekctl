...
cv::Rect IDocument::getFrame(cv::MatSize imageSize, float p)
{
...
bool IDocument::irClusterCheck(cv::Mat image, int MaxClusters, float p)
{
...
bool IDocument::isInsideRect(cv::Rect roi, cv::Point point, cv::MatSize image_size)
{
    int th = image_size[0];
    int tw = image_size[1];    
    float maxh = roi.y;
...
float IDocument::getUvPercentage(cv::Mat mat)
{
    cv::Mat hsv, thrsd;
...
float IDocument::getIrBrightness(cv::Mat irImage)
{
    cv::Mat hsv;
